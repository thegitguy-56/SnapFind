  ...
  Future<void> _handleSignIn() async {
    ...
    try {
      ...
      if (user != null && mounted) {
        ...
        _showStatus('Signed in as ${user.email}', Colors.green.shade600, icon: Icons.check_circle_rounded);
      }
    } catch (e) {
      if (mounted) {
        _showStatus('Sign-in failed: $e', Colors.red.shade600, icon: Icons.error_outline);
      }
    } finally {
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            gradient: LinearGradient(
                              ...
                              colors: [
                                  Color(0xFFC8D9E6).withAlpha(170),
                                  Color(0xFFC8D9E6).withAlpha(110),
                              ],
                            ),
                            borderRadius: BorderRadius.circular(30),
                            border: Border.all(color: Color(0xFFC8D9E6).withAlpha(140)),
                            boxShadow: [
                              BoxShadow(
                            ...
                            children: [
                              ...
                              Text(
                                ...
                                style: Theme.of(context).textTheme.titleMedium?.copyWith(
                                    color: _textPrimary.withAlpha(189),
                              ...
                              DecoratedBox(
                                ...
                                child: FilledButton.icon(
                                  ...
                                  label: Text(_loading ? 'Signing you in...' : 'Continue with Google'),
                                ),
                              ),
                              ...
                              Opacity(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Icon(Icons.verified_user_rounded, color: _textPrimary.withOpacity(0.8), size: 18),
                                    const SizedBox(width: 6),
                                    Text(
                                      'Secure sign-in • Your email stays private',
                                      style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                                            color: _textPrimary.withAlpha(184),
                                            fontWeight: FontWeight.w600,
                                          ),
                                    ),
                                  ],
...
class _MapOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = -40; x < size.width + 40; x += gap) {
      ...
      canvas.drawLine(Offset(x + gap / 2, 0), Offset(x + gap / 2 - 40, size.height), paint);
    }

    for (double y = 0; y < size.height + 80; y += gap) {
      canvas.drawLine(Offset(0, y), Offset(size.width, y + 30), paint);
      canvas.drawLine(Offset(0, y + gap / 2), Offset(size.width, y + gap / 2 - 30), paint);
    }
    ...
    for (double x = 20; x < size.width; x += gap) {
      for (double y = 30; y < size.height; y += gap) {
        canvas.drawCircle(Offset(x + (y % 2 == 0 ? 10 : -10), y), 3, nodePaint);
      }
    }
  }

  @override
  bool shouldRepaint(covariant _MapOverlayPainter oldDelegate) => oldDelegate.color != color;
}
